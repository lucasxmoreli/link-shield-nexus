const en = {
  // Common
  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    create: "Create",
    copy: "Copy",
    copied: "Copied!",
    loading: "Loading...",
    back: "Back",
    close: "Close",
    confirm: "Confirm",
    search: "Search",
    clear: "Clear",
    upgrade: "Upgrade",
    add: "Add",
    remove: "Remove",
    verify: "Verify",
    saving: "Saving...",
    adding: "Adding...",
    testing: "Testing...",
    refresh: "Refresh",
    actions: "Actions",
    status: "Status",
    active: "Active",
    inactive: "Inactive",
    yes: "Yes",
    no: "No",
    or: "or",
    of: "of",
    live: "LIVE",
    realTime: "Real-time",
    monitoring: "Monitoring",
    signOut: "Sign Out",
    goBack: "Go Back & Fix",
    saveAnyway: "Save Anyway",
    returnHome: "Return to Home",
    advanced: "Advanced",
  },

  // Sidebar navigation
  nav: {
    dashboard: "Dashboard",
    domains: "Domains",
    campaigns: "Campaigns",
    requests: "Requests",
    analytics: "Analytics",
    cloakTest: "Cloak Test",
    billing: "Billing & Plans",
    settings: "Settings",
    adminUsers: "Admin & Users",
    adminCommandCenter: "Admin Center",
  },

  // Dashboard
  dashboard: {
    title: "Command Center",
    subtitle: "Real-time traffic intelligence & threat monitoring",
    totalRequests: "Total Requests",
    passRate: "Pass Rate",
    offerPage: "Offer Page",
    botsBlocked: "Bots Blocked",
    threatsBlocked: "Threats Blocked",
    trafficOverview: "Traffic Overview",
    trafficByDevice: "Traffic by Device",
    liveTrafficLog: "Live Traffic Log",
    today: "Today",
    last7Days: "Last 7 Days",
    last30Days: "Last 30 Days",
    time: "Time",
    ipAddress: "IP Address",
    country: "Country",
    device: "Device",
    action: "Action",
    passed: "Passed",
    blocked: "Blocked",
    desktop: "Desktop",
    mobile: "Mobile",
    tablet: "Tablet",
    realTrafficRatio: "Real traffic ratio",
    allRejected: "All rejected traffic",
    allTime: "All Time",
    topAttackOrigins: "Top Attack Origins",
    globalShield: "Global Shield",
    noAttackData: "No attacks recorded yet.",
    healthCalculating: "Calculating...",
    // V2 Command Center
    justNow: "just now",
    lastUpdated: "Updated",
    yesterday: "Yesterday",
    refresh: "Refresh data",
    metricsLabel: "Metrics",
    botsBlockedSub: "identified threats",
    safePageHits: "Safe Page Hits",
    safePageHitsSub: "cloaked redirects",
    realTraffic: "Real Traffic",
    realTrafficSub: "clean visitors",
    totalRequestsSub: "all sources today",
    shieldActive: "Shield Active",
    shieldStats: "99.98% uptime · 47ms avg",
    trendNew: "NEW",
    trafficFlow: "Traffic Flow",
    hourly: "Hourly",
    daily: "Daily",
    noTrafficYet: "No traffic yet — shield armed.",
    shieldWatching: "When your links start spinning, the chart comes alive here in real time.",
    // [PR-3d.2] Magnetic LiveStream empty state
    liveStreamEmptyTitle: "Waiting for the first click...",
    liveStreamEmptyDesc: "Every visitor will appear here in real time — IP, country, device and shield verdict.",
    realLabel: "Real",
    blockedLabel: "Blocked",
    liveStream: "Live Stream",
    noTrafficDetected: "No traffic detected yet.",
    statusClean: "CLEAN",
    statusBlocked: "BLOCKED",
    viewFullLog: "View full log",
  },

  // Volatility Radar
  volatility: {
    networkStatus: "📡 Network Status:",
    stable: "Stable",
    highVolatility: "High Volatility",
    metaHelper: "Normal approval rates",
    tiktokHelper: "Ban wave detected. Scale cautiously.",
    googleHelper: "No anomalies",
  },

  // Threat Interceptions
  threats: {
    title: "Live Threat Interceptions",
    botBlocked: "Bot blocked",
    safePageRedirect: "Redirected to safe page",
    intercepted: "Threat Intercepted",
    noThreats: "No threats intercepted yet.",
    noThreatsHelper: "Threats will appear here in real-time.",
    on: "on",
    justNow: "Just now",
  },

  // Dopamine Toast
  dopamine: {
    title: "🛡️ Shield Active",
    description: "CloakerX successfully intercepted {{count}} threats in the last 24 hours.",
    topThreat: "Top threat: {{reason}}",
  },

  // Campaigns
  campaigns: {
    title: "Campaigns",
    createNew: "Create +",
    upgradeToCreate: "Upgrade to Create",
    limitReached: "Plan limit reached",
    gatePaidPlan: "Campaign creation is a paid-plan feature.",
    gatePaidPlanShort: "Paid-plan feature",
    gatePastDue: "Payment pending — settle your invoice to keep creating.",
    gatePastDueShort: "Payment pending",
    gateCanceled: "Subscription ended.",
    gateCanceledShort: "Subscription ended",
    badgeAtCap: "Maximum for {{plan}}",
    badgeOverLimit: "Above {{plan}} limit · {{used}} of {{effective}}",
    viewOnlyHint: "View-only mode — this tenant's limits aren't available here.",
    viewOnlyAdd: "View only",
    viewOnlyMode: "Free plan does not include campaign creation. Upgrade to start protecting your traffic.",
    noCampaigns: "No campaigns created yet.",
    // [PR-3d.2] Magnetic Campaigns empty state
    emptyTitle: "Ready to shield your first offer?",
    emptyDesc: "Each campaign is a bulletproof link against bots, Meta/TikTok spies, and platform audits. Setup in 1 minute.",
    emptyCta: "Create my first campaign",
    emptyTitleLocked: "Unlock campaign creation",
    emptyDescLocked: "Upgrade your plan and start protecting your traffic today.",
    emptyCtaLocked: "View plans",
    hash: "Hash",
    name: "Name",
    source: "Source",
    date: "Date",
    campaignLink: "Campaign Link",
    domain: "Domain",
    campaignUrl: "Campaign URL",
    copyLink: "Copy Link",
    campaignLinkCopied: "Campaign link copied!",
    macrosHint: "The macros will be automatically replaced by the ad platform when it receives the click.",
    passthroughHint: "Tip: You can append external tracker parameters (like RedTrack or Voluum) directly to the end of this link. Our system will automatically pass them through to your landing page.",
    webhookTitle: "Sales Webhook (Checkout Integration)",
    webhookDescription: "Copy the URL below and set it up in your payment platform (Kiwify, Hotmart, PerfectPay, etc.) to receive revenue and ROI data directly in your Analytics dashboard.",
    copyWebhook: "Copy Webhook",
    webhookCopied: "Webhook URL copied!",
    selectDomainFirst: "Select a domain above to generate the webhook URL.",
    campaignRemoved: "Campaign removed",
    deleteConfirmTitle: "Delete campaign",
    deleteConfirmMessage: "Are you sure you want to delete the campaign {{name}}? This action cannot be undone.",
    deleteConfirmCancel: "Cancel",
    deleteConfirmAction: "Delete",
    noDomainsWarning: "⚠️ No domains available. You must connect and verify a custom domain in the Domains tab before generating a campaign link.",
    goToDomains: "Go to Domains",
    dnsReminder: "Make sure your domain has a CNAME record pointing to cname.cloakerx.com with Cloudflare proxy active (orange cloud).",
    errors: {
      limitReached: "You've reached your plan's campaign limit.",
      domainNotOwned: "That domain is not registered on your account.",
      domainNotVerified: "That domain is not verified yet.",
      saveFailed: "Couldn't save the campaign. Please try again.",
    },
  },

  // Campaign Edit / New
  campaignEdit: {
    newCampaign: "New Campaign",
    editCampaign: "Edit Campaign",
    campaignSection: "Campaign",
    campaignName: "Campaign Name",
    campaignNamePlaceholder: "e.g. TTK 10 - FREE [TRESH-$500]",
    domainLabel: "Domain",
    domainHelper: "Your domain must have a CNAME record → cname.cloakerx.com with Cloudflare proxy active.",
    domainTooltip: 'Example: If you select "track.mysite.com", your link will be "track.mysite.com/c/xyz". Set up a CNAME record pointing to cname.cloakerx.com with the orange cloud active on Cloudflare.',
    noDomains: "No verified domains found.",
    noDomainsBlock: "⚠️ No domains available. You must connect and verify a custom domain in the Domains tab before creating a campaign.",
    noDomainsAction: "Go to the Domains tab",
    noDomainsHelper: "to add one first.",
    selectDomain: "Select a domain",
    domainSslActive: "SSL Active",
    domainSslPending: "SSL Provisioning",
    domainSslWarning: "SSL is being provisioned by Cloudflare. This is normal and does not prevent the campaign from working. Wait up to 15 minutes after activating the orange cloud in DNS.",
    trafficSource: "Traffic Source",
    selectSource: "Select source",
    unlockMore: "Upgrade plan to unlock more sources",

    safePageSection: "🛡️ Safe Page",
    safePageSectionDesc: "The page shown to bots, reviewers, and non-targeted traffic.",
    safePageUrl: "Safe Page URL",
    safePagePlaceholder: "https://blog.example.com/...",

    offerPageSection: "💰 Offer Page",
    offerPageSectionDesc: "The real money page. Only shown to real, targeted visitors.",
    primaryOffer: "Primary Offer Page (A)",
    offerPlaceholder: "https://offer.example.com/...",
    abStormTitle: "⚡ A/B Storm (Split Traffic)",
    abStormDesc: "Split human traffic 50/50 between two offers",
    testOfferB: "Test Offer Page (B)",
    testOfferPlaceholder: "https://alternative-offer.example.com/...",
    abStormHelp: "When enabled, CloakerX will automatically split your approved human traffic <bold>50/50</bold> between Offer A and Offer B. Bots will still be sent to the Safe Page.",

    prelanderSection: "📄 Pre-Lander",
    prelanderEnabledTitle: "Enable Pre-Lander",
    prelanderEnabledDesc: "Show an intermediate page with legitimate content before the offer. Recommended for sensitive niches.",
    prelanderHeadlineLabel: "Pre-Lander headline",
    prelanderHeadlinePlaceholder: "e.g. Men's Health — Before You Continue",
    prelanderHeadlineRequired: "Pre-Lander headline is required",
    prelanderFieldsRequired: "Fill in the required Pre-Lander fields",
    prelanderBodyLabel: "Pre-Lander text",
    prelanderBodyPlaceholder: "Text shown to the visitor while we verify their connection. Avoid mentioning the product directly.",
    prelanderBodyRequired: "Pre-Lander text is required",
    prelanderCtaLabel: "Button text",
    prelanderCtaPlaceholder: "Continue",
    powDifficultyLabel: "Verification difficulty",
    powDifficulty3Helper: "Fast (~1s) — less strict",
    powDifficulty4Helper: "Balanced (~2-3s) — recommended",
    powDifficulty5Helper: "Strict (~5-8s) — may affect conversion",

    behavioralModeSection: "Protection mode",
    behavioralModeProtectTitle: "Protect leads (recommended)",
    behavioralModeProtectDesc: "Prioritizes not blocking real users. Best for most niches.",
    behavioralModeAggressiveTitle: "Anti-reviewer (aggressive)",
    behavioralModeAggressiveDesc: "Stronger protection against platform reviewers. May block passive humans.",
    behavioralModeAggressiveWarning: "Recommended only with Pre-Lander enabled.",

    securitySection: "Security",
    strictModeTitle: "🛡️ Strict Mode",
    strictModeDesc: "Block suspicious traffic (missing click IDs, unknown referers)",

    targetSection: "Target",
    tiktokWarning: "For TikTok campaigns, we recommend leaving this field empty (Global Traffic).",
    countries: "Countries",
    searchCountries: "Select countries or leave empty for all...",
    devices: "Devices",

    tagsSection: "Tags",
    tagHelper: "Press Enter to add a tag",
    tagPlaceholder: "Type a tag and press Enter...",

    saveCampaign: "Save Campaign",
    campaignUpdated: "Campaign updated!",
    campaignCreated: "Campaign created!",

    // Conflict dialog
    conflictTitle: "⚠️ Warning: Same Domain Detected",
    conflictDesc: "You are using the same domain ({{domain}}) for your tracking link and your destination page. This will cause a 404 error unless you are using a specific subdirectory.",
    conflictRecommend: "We strongly recommend using a different domain or an external link for your Offer Page and Safe Page.",

    // Success modal
    successTitle: "🚀 Campaign Saved Successfully!",
    successHelp: "Copy the Final URL below and paste it directly into your ad platform to ensure perfect tracking.",
    finalUrl: "Campaign Final URL",
    copyFinalUrl: "Copy Final URL",
    closeCampaigns: "Close and go to Campaigns",
    trackingLink: "Your Tracking Link",
    offerPageLabel: "Offer Page",
    safePageLabel: "Safe Page",
    quickSetup: "Quick Setup Guide",
    step1: "Copy this link and use it in your <bold>Ad Manager</bold> (Facebook, TikTok, Google, etc).",
    step2: "Make sure your domain has a <bold>CNAME record pointing to cname.cloakerx.com</bold> with Cloudflare proxy (orange cloud) active.",
    step3: "Test your link using a <bold>VPN or Incognito window</bold> to verify the redirection works correctly.",
    goToCampaigns: "Go to Campaigns",
    linkCopied: "Link copied!",
    testRecommendation: "Test this link using a VPN or incognito window to verify the cloaking works correctly.",
    invalidUrl: "Please enter a valid destination URL.",
    webhookTitle: "WEBHOOK POSTBACK",
    webhookDesc: "Fired when a lead is approved. Build your tracker URL visually using macros.",
    customValue: "Custom",
    webhookBaseUrl: "Base URL",
    webhookParams: "Query Parameters",
    paramKey: "key",
    paramValue: "custom value",
    addParam: "Add Parameter",
    selectMacro: "Select macro…",
    methodLabel: "Method",
    webhookMethod: "HTTP Method",
    webhookMethodDesc: "Use GET for trackers (RedTrack, UTMify, BeMob). Use POST for Conversion APIs (Facebook CAPI, TikTok Events).",
    methodHint: "Use GET for external trackers (RedTrack, UTMify, BeMob). Use POST for Facebook CAPI or TikTok Events API.",
  },

  // Domains
  domains: {
    title: "Domains",
    addDomain: "Add Domain",
    limitReached: "Limit Reached (Upgrade)",
    domainUsage: "Domain Usage",
    domainsUsed: "Domains Used",
    noPlanDomains: "Your current plan does not include custom domains. Upgrade to unlock this feature.",
    gatePaidPlan: "Custom domains are a paid-plan feature.",
    gatePaidPlanShort: "Paid-plan feature",
    gatePastDue: "Payment pending — settle your invoice to keep creating.",
    gatePastDueShort: "Payment pending",
    gateCanceled: "Subscription ended.",
    gateCanceledShort: "Subscription ended",
    gateOverLimit: "Above your plan limit — remove extras or upgrade.",
    gateOverLimitDetail: "{{used}} of {{effective}} — remove the excess or buy slots (soon).",
    badgeAtCap: "Maximum for {{plan}}",
    badgeOverLimit: "Above {{plan}} limit · {{used}} of {{effective}}",
    upgradeToDomains: "Upgrade to add domains",
    viewOnlyHint: "View-only mode — this tenant's limits aren't available here.",
    viewOnlyAdd: "View only",
    addDomainTitle: "Connect Your Domain",
    addDomainDesc: "We recommend using a subdomain (e.g. app.yourdomain.com) for the smoothest setup.",
    domainUrl: "Domain",
    domainUrlPlaceholder: "app.yourdomain.com",
    domainAdded: "Domain registered! Follow the steps below to activate.",
    domainRemoved: "Domain removed",
    domainRemovedCloudflare: "Domain removed from Cloudflare and system.",
    domainDuplicate: "This domain has already been added.",
    domainRequired: "Domain URL is required.",
    setupDomain: "Connect Domain",
    setupDomainDesc: "Follow these 3 steps to connect",
    checkStatus: "Verify",
    checking: "Verifying...",
    verified: "Active",
    pending: "Pending",
    domainActive: "Domain is active and ready for traffic!",
    domainPending: "Domain is still pending. Make sure your CNAME record points to cname.cloakerx.com with Cloudflare proxy (orange cloud) active.",
    domainNotVerified: "Domain not registered. Check if it's in the Domains tab.",
    verificationFailed: "We couldn't verify your domain. Wait a few minutes and try again.",
    dnsNotPointing: "DNS not pointing to the server yet. Check your CNAME record in Cloudflare.",
    url: "URL",
    created: "Created",
    sslStatus: "SSL",
    valueCopied: "Value copied!",
    dnsStep1: "Log in to Cloudflare and add your domain (Free plan works).",
    dnsStep2: "Create a CNAME record pointing to:",
    dnsStep2Note: "Make sure the Proxy (orange cloud ☁️) is ON.",
    dnsStep3: "Save your DNS changes and click Add below.",
    learnMoreCf: "Learn more about Cloudflare",
    dnsTable: "DNS Configuration",
    dnsType: "Type",
    dnsName: "Name",
    dnsContent: "Content",
    dnsProxy: "Proxy",
    dnsProxyActive: "Active (orange cloud)",
    dnsSetupTitle: "How to configure your DNS",
    dnsNameHint: "or @ for root domain",
    dnsProxyWarning: "The orange cloud (proxy) is mandatory. Without it, CloakerX may not work.",
    dnsError: "DNS Error",
    verifying: "Verifying",
    cfWhyTitle: "Why is Cloudflare mandatory?",
    cfBanWaveTitle: "Ban Wave Protection",
    cfBanWaveDesc: "Cloudflare hides your server's real IP. Without it, ad networks like Meta and TikTok can identify and ban your infrastructure directly, taking down all campaigns at once.",
    cfSecurityTitle: "Cloaking Engine Security",
    cfSecurityDesc: "CloakerX validates that all traffic passes through Cloudflare before reaching the engine. This prevents bots, moderators, and crawlers from accessing your real page directly.",
    cfSslTitle: "Free SSL & DDoS Protection",
    cfSslDesc: "Cloudflare provides a free SSL certificate and automatic DDoS protection at no additional cost to you.",
    cfWarning: "Domains without active Cloudflare (orange cloud) may not work with CloakerX.",
    // Domains page section labels + empty state + toasts
    awaitingSetup: "Awaiting setup",
    activeDomains: "Active domains",
    emptyStateTitle: "No domains connected yet.",
    emptyStateSubtitle: "Add a custom domain above to start serving cloaked traffic.",
    // [PR-3d.2] Magnetic Domains empty state
    emptyTitle: "Connect your first domain",
    emptyDesc: "No domain = no clean traffic. Setup in 2 minutes with auto SSL via Cloudflare and start protecting your links now.",
    emptyCta: "Add my first domain",
    emptyTitleLocked: "Current plan doesn't include domains",
    emptyDescLocked: "Upgrade to unlock custom domains and shield your offers.",
    emptyCtaLocked: "View plans",
    domainAddedConfigureDns: "Domain added. Configure the DNS records below.",
    verifiedSslActive: "Domain verified and SSL active.",
    cnameNotPointing: "CNAME is not pointing to cname.cloakerx.com yet.",
    waitingSsl: "Waiting for SSL",
    flowDiagram: {
      title: "Traffic path",
      subtitle: "Each step lights up green once DNS propagates.",
      ariaLabel: "Diagram of the DNS flow from your domain to the CloakerX engine",
      nodeVisitor: "Visitor",
      nodeVisitorSub: "Clicks the link",
      nodeDomain: "Your domain",
      nodeCname: "CNAME record",
      nodeCloudflare: "Cloudflare",
      nodeCloudflareSub: "SSL + Anti-DDoS",
      nodeEngine: "CloakerX",
      nodeEngineSub: "Cloaking + Metrics",
      summaryVerified: "✓ All set. Your domain is active and routing traffic.",
      summaryCnameMissing: "Configure the CNAME at your DNS provider (records below). Propagation takes 1 to 15 minutes.",
      summarySslPending: "CNAME resolved. Cloudflare is issuing the SSL certificate — this can take up to 15 minutes.",
      summaryWaiting: "Waiting for the first verification. Click \"Verify now\" after setting up the DNS records.",
    },
  },

  // ROI Hero card (Analytics)
  roiHero: {
    moneySaved: "Money Saved",
    botsByCpc: "bots ×",
    estimatedTag: "estimated",
    cpcEstimated: "Estimated CPC",
    revenueLabel: "Revenue",
    costLabel: "Cost",
    noCostData: "Cost data unavailable",
    tooltipEstimated:
      "Estimated value — blocked clicks multiplied by your average CPC. Adjust the value next to it to reflect your actual cost.",
    tooltipCalculated:
      "Calculated automatically from the real cost tracked by your pipeline divided by approved clicks.",
  },

  // Requests Log
  requests: {
    title: "Requests Log",
    searchPlaceholder: "Search IP or Hash...",
    allActions: "All Actions",
    passedOffer: "Passed (Offer)",
    allCampaigns: "All Campaigns",
    allDevices: "All Devices",
    allTime: "All Time",
    days7: "7 Days",
    days30: "30 Days",
    showing: "Showing {{count}} of {{total}} requests",
    createdAt: "Created At",
    campaign: "Campaign",
    ip: "IP",
    reason: "Reason",
    noRequests: "No requests found matching these filters.",
    clearAllFilters: "Clear all filters",
    safePage: "Safe Page",
    score: "Score",
    scoreLow: "Low",
    scoreMedium: "Medium",
    scoreHigh: "High",
    globalBlacklist: "🌐 Global",
    userBlocked: "Blocked",
    globalBlacklistCount: "{{count}} IPs on global blacklist",
    conversion: "Conversion",
    allClicks: "All Clicks",
    salesOnly: "Sales Only",
    sale: "Sale",
  },

  // Cloak Test
  cloakTest: {
    title: "Cloak Test",
    subtitle: "Simulate requests to test the cloaking filter in real time",
    testConfig: "Test Configuration",
    testConfigDesc: "Define the parameters for the simulated request",
    campaignRequired: "Campaign *",
    selectCampaign: "Select a campaign",
    userAgentRequired: "User Agent *",
    choosePreset: "Choose a preset",
    customUA: "Or type a custom user agent...",
    ipRequired: "IP Address *",
    enterIp: "Enter IP (e.g. 192.168.1.1)",
    refererOptional: "Referer (optional)",
    runTest: "Run Test",
    results: "Results",
    testsExecuted: "{{count}} test(s) executed",
    noTests: "No tests executed yet",
    noTestsHelper: 'Configure the parameters and click "Run Test"',
    fillRequired: "Please fill in all required fields",
    testError: "Test error: {{message}}",
    logCopied: "Log copied!",
    resultApproved: "Approved — Offer Page",
    resultBlocked: "Blocked — Safe Page",
    simulatedMode: "Simulated Mode",
    simulatedTooltip: "In simulated mode, no external APIs are called. Use Real Mode to test with full IP verification.",
    realModeWarning: "This test consumes IP verification API credits.",
    badgeSimulated: "Simulated",
    badgeReal: "Real",
  },

  // Billing
  billing: {
    title: "Billing & Plans",
    starterPlans: "Starter Plans",
    scaleEnterprise: "Scale & Enterprise",
    currentPlan: "Current Plan",
    downgrade: "Downgrade",
    yourPlan: "YOUR PLAN",
    whatsIncluded: "What's included",
    includedSources: "Included Traffic Sources",
    selectedPlan: "SELECTED PLAN",
    currentPlanValid: "Current plan valid until",
    // ── Stripe Checkout ──
    freePlanInfo: "Free plan",
    freePlanDescription: "You are already on the free plan. Choose a paid plan to unlock features.",
    proceedToCheckout: "Proceed to checkout",
    redirecting: "Redirecting...",
    checkoutFailed: "Checkout failed",
    checkoutFailedDesc: "Could not connect to the payment gateway. Please try again in a moment.",
    checkoutSuccess: "Payment confirmed",
    checkoutSuccessDesc: "Your plan has been activated. You can start using it now.",
    checkoutCancelled: "Checkout cancelled",
    checkoutCancelledDesc: "No charge was made. You can try again whenever you want.",
    // ── Sprint 6: Client Portal MVP ──
    tabAccount: "My Account",
    tabPlans: "Plans",
    tabPacks: "Packs",
    checkoutComingSoon: "Coming soon",
    needBannerPlans: "To add {{resource}} you need a paid plan.",
    needResourceDomain: "domains",
    needResourceCampaign: "campaigns",
    needResourceClicks: "clicks",

    packs: {
      headerTitle: "Plan usage and headroom",
      headerSubtitle: "Effective limits for {{plan}} · maximum = pack ceiling",
      clicksTitle: "Clicks",
      domainsTitle: "Domains",
      campaignsTitle: "Campaigns",
      quotaUsed: "{{bought}} of {{max}} packs this cycle",
      capacityUsed: "{{used}} of {{effective}} · max {{cap}}",
      renewsOn: "renews {{date}}",
      unitClicks: "+{{n}} clicks",
      unitSlotDomain: "+1 domain",
      unitSlotCampaign: "+1 campaign",
      comingSoon: "Coming soon",
      buyCta: "Buy {{price}}/mo",
      buySuccessTitle: "Pack added",
      buySuccessDesc: "Your subscription was updated.",
      buyErrorTitle: "Could not buy pack",
      atCap: "Maximum for {{plan}}",
      atCapClicks: "Maximum for {{plan}}: {{cap}} clicks",
      cycleQuota: "Click pack quota used for this cycle.",
      unlimited: "Unlimited on your plan",
      overLimitRemove: "Remove {{n}} to get back under the limit",
      overLimitBuy: "or buy {{n}} slot(s) when available",
      viewPlans: "View plans",
      backToDomains: "Back to Domains",
      backToCampaigns: "Back to Campaigns",
      backToDashboard: "Back to dashboard",
      inactivePastDue: "Payment pending — settle to buy packs.",
      inactiveCanceled: "Subscription ended.",
      inactivePastDueCta: "Settle to purchase",
      inactiveCanceledCta: "Subscription ended",
      bypassManaged: "Plan managed by support",
      badgeAtLimit: "Limit reached",
      badgeOverLimit: "Over limit",
    },

    // Plan Overview Card
    planCardTitle: "Your current plan",
    changePlanButton: "Change plan",
    billingCycleLabel: "Current cycle",
    daysRemaining: "in {{days}} days",
    daysRemainingOne: "in 1 day",
    daysRemainingZero: "ends today",
    clicksUsedLabel: "clicks used",
    usageWithinPlan: "Usage within plan",
    perMonth: "/month",

    // Overage Card
    overageAlertTitle: "You exceeded your plan limit",
    overageClicksLabel: "extra clicks",
    overageCostLabel: "{{cost}} will be charged on next billing",
    overageRateLabel: "Plan rate: {{rate}} per extra click",
    overageViewDetails: "View usage details",

    // Limits Grid
    limitsGridTitle: "Plan limits",
    domainsLabel: "Domains",
    campaignsLabel: "Campaigns",
    addSlotButton: "+ Add slot",
    limitsIncludesAddons: "(+ {{extra}} addon)",
    limitsIncludesAddonsPlural: "(+ {{extra}} addons)",
    // ── Sprint 6 Bloco B: Faturas + Payment Method ──
    tabFaturas: "Invoices",

    paymentMethodTitle: "Payment method",
    cardExpires: "Expires {{date}}",
    manageSubscription: "Manage on Stripe",
    noCardLinked: "No card linked",
    noCardLinkedDesc: "Subscribe to a plan to add a payment method",
    portalFailed: "Could not open portal",
    portalFailedDesc: "Please try again in a few seconds",

    invoicesTitle: "Invoice history",
    invoicesCount: "{{count}} invoice",
    invoicesCountPlural: "{{count}} invoices",
    invoicesEmptyTitle: "No invoices yet",
    invoicesEmptyDesc: "Your first charge will appear here when the billing cycle closes.",
    invoiceColDate: "Date",
    invoiceColDescription: "Description",
    invoiceColAmount: "Amount",
    invoiceColStatus: "Status",
    invoiceColActions: "Actions",
    invoiceStatusPaid: "Paid",
    invoiceStatusFailed: "Failed",
    invoiceStatusPending: "Pending",
    invoiceViewTooltip: "View invoice on Stripe",
    // ── Sprint 6 Bloco C: Polimento ──
    usageOverPlan: "Usage exceeds plan limit",
    invoiceDescWithOverage: "{{plan}} · base + extra usage",
    overageEstimatedCost: "Estimated extra cost: +{{clicks}} clicks ·",
    // ── Addon Confirmation Dialog ──
    addonConfirmTitle: "Confirm slot addition",
    addonConfirmDesc: "You are adding 1 extra {{addon}} slot to your subscription.",
    addonDomainLabel: "Domain",
    addonCampaignLabel: "Campaign",
    addonRecurringCost: "Monthly recurring cost",
    addonProrationWarning: "A prorated charge may be applied immediately to your credit card for the remaining days in the current billing cycle. Starting next cycle, the full amount will be charged monthly.",
    addonCancel: "Cancel",
    addonConfirmButton: "Confirm payment",
    addonProcessing: "Processing...",
    addonGenericError: "Could not add the slot. Please try again.",
  },

  // Settings
  settings: {
    title: "Account Settings",
    profile: "Profile",
    email: "Email",
    plan: "Plan",
    planUsage: "Plan Usage",
    clicksUsed: "Clicks used",
    ofLimitUsed: "{{percent}}% of limit used",
    domainsUsed: "Domains used",
    upgradeForDomains: "Upgrade to unlock custom domains",
    campaignsCreated: "Campaigns created",
    unlimitedAccess: "Unlimited Access",
    upgradeForCampaigns: "Upgrade to create campaigns",
    seePlans: "SEE PLANS & UPGRADE",
    language: "Language",
    languageDesc: "Select your preferred language",
    freePlanStatus: "Free Plan",
    noClicksIncluded: "Upgrade to unlock click tracking",
    noCampaignsIncluded: "Upgrade to create campaigns",
    noDomainsIncluded: "Upgrade to add custom domains",
  },
  profile: {
    pageTitle: "Account Settings",
    pageSubtitle: "Manage your profile information and security",

    profileSectionTitle: "Profile",
    emailLabel: "Email",
    emailReadOnlyHint: "Email cannot be changed",
    displayNameLabel: "Display name",
    displayNamePlaceholder: "How you want to be called",
    displayNameHint: "Between 2 and 50 characters",
    saveChanges: "Save changes",
    saving: "Saving...",
    profileUpdatedTitle: "Profile updated",
    profileUpdatedDesc: "Your information has been saved successfully",
    profileUpdateFailed: "Could not save",
    profileUpdateFailedDesc: "Please try again in a few seconds",
    displayNameTooShort: "Name must be at least 2 characters",
    displayNameTooLong: "Name must be at most 50 characters",

    securitySectionTitle: "Security",
    securitySectionDesc: "Change your access password",
    newPasswordLabel: "New password",
    newPasswordPlaceholder: "Enter your new password",
    confirmPasswordLabel: "Confirm new password",
    confirmPasswordPlaceholder: "Enter the password again",
    updatePassword: "Update password",
    updatingPassword: "Updating...",
    passwordStrengthWeak: "Weak",
    passwordStrengthMedium: "Medium",
    passwordStrengthStrong: "Strong",
    passwordStrengthLabel: "Strength",
    passwordUpdatedTitle: "Password updated",
    passwordUpdatedDesc: "Use your new password on next login",
    passwordUpdateFailed: "Could not change password",
    passwordUpdateFailedDesc: "Please try again in a few seconds",
    passwordTooShort: "Password must be at least 8 characters",
    passwordNeedsUppercase: "Password must contain at least 1 uppercase letter",
    passwordNeedsLowercase: "Password must contain at least 1 lowercase letter",
    passwordsDoNotMatch: "Passwords do not match",
    showPassword: "Show password",
    hidePassword: "Hide password",
    tabProfile: "Profile",
    tabSecurity: "Security",
    tabDanger: "Danger Zone",

    profileTitle: "Profile",
    profileSubtitle: "Update your profile information and email preferences.",
    securityTitle: "Security",
    securitySubtitle: "Manage your password and security preferences.",
    dangerTitle: "Danger Zone",
    dangerSubtitle: "Irreversible and destructive actions for your account.",
    dangerComingSoon: "Danger zone settings coming soon",
},
  password: {
    criteriaMinLength: "At least 6 characters",
    criteriaHasLetter: "At least 1 letter",
    criteriaHasNumber: "At least 1 number",

    strengthLabel: "Requirements",
    strengthWeak: "Incomplete",
    strengthMedium: "Almost",
    strengthStrong: "Requirements met",
    breachHint: "Common passwords or ones found in public breaches will be rejected by the server.",

    notAcceptable: "All criteria must be green",
    doNotMatch: "Passwords do not match",
},
  onboarding: {
    welcomeTitle: "Welcome to CloakerX!",
    welcomeDesc: "Complete these 3 steps to start protecting your traffic.",
    step1Title: "Add a Domain",
    step1Desc: "Connect your custom domain with Cloudflare DNS.",
    step2Title: "Create a Campaign",
    step2Desc: "Set up your offer and safe pages.",
    step3Title: "Go Live",
    step3Desc: "Activate your campaign and start filtering traffic.",
    completed: "Completed",
    pending: "Pending",
    letsGo: "Let's Go",
    skipForNow: "Skip for now",
  },
  dangerZone: {
    sectionTitle: "Danger Zone",
    sectionDesc: "Irreversible and destructive actions for your account.",
    
    consequencesTitle: "When you delete your account:",
    consequence1: "Your subscription will be canceled immediately, with no refund",
    consequence2: "All your domains and campaigns will be deactivated",
    consequence3: "You'll have 30 days to reactivate by contacting support",
    consequence4: "After 30 days, all your data will be permanently deleted",
    
    deleteButton: "Delete my account",
    
    dialogTitle: "Are you absolutely sure?",
    dialogWarning: "This action cannot be undone through the interface. After deletion, only support can reactivate your account.",
    dialogGracePeriod: "You'll have 30 days to contact support@cloakerx.com before everything is permanently erased.",
    typeToConfirm: 'Type "{{text}}" to confirm',
    confirmText: "DELETE MY ACCOUNT",
    confirmTextMismatch: "Text doesn't match",
    
    cancel: "Cancel",
    confirmDelete: "Yes, delete account",
    deleting: "Deleting...",
    
    deleteSuccessTitle: "Account deleted",
    deleteSuccessDesc: "You'll be signed out shortly.",
    deleteFailedTitle: "Could not delete",
    deleteFailedDesc: "Please try again or contact support.",
  },

  accountDeleted: {
    title: "Account Deactivated",
    description: "Your CloakerX account has been deactivated and is marked for permanent deletion in 30 days.",
    howToRestore: "How to restore",
    howToRestoreDesc: "Contact support within 30 days to reactivate your account. After this period, all data will be permanently removed.",
    contactSupport: "Contact support",
    backToHome: "Back to home",
    gracePeriodNote: "Your account will be permanently deleted within 30 days.",
},
  // Auth
  auth: {
    welcomeBack: "Welcome back",
    welcomeSubtitle: "Enter your credentials to access the dashboard.",
    inviteCode: "Invite Code",
    inviteSubtitle: "Enter your invite code to create an account.",
    createAccount: "Create your account",
    createSubtitle: "Start protecting your campaigns in minutes.",
    iHaveInvite: "I have an invite",
    alreadyHaveAccount: "Already have an account?",
    signIn: "Sign in",
    emailLabel: "Email",
    emailPlaceholder: "you@email.com",
    passwordLabel: "Password",
    passwordPlaceholder: "••••••••",
    signInButton: "Sign In",
    inviteCodeLabel: "Invite Code",
    inviteCodePlaceholder: "CLOAKER-XXXX-XXXX",
    invalidInvite: "This invite code is invalid or has already been used.",
    validateCode: "Validate Code",
    validCode: "Valid code! Create your account.",
    createAccountButton: "Create Account",
    minChars: "Minimum 6 characters",
    inviteConsumed: "Invite code was already consumed. Please contact the administrator.",
    accountCreated: "Account created! Welcome to CloakerX.",
    registrationFailed: "Unable to create the account. Please try again.",
    passwordPwned: "That password has appeared in data breaches. Choose a unique password you have never used elsewhere.",
    forgotPassword: "Forgot your password?",
    forgotPasswordInvalidEmail: "Please enter a valid email above first.",
    forgotPasswordSent: "If this email is registered, a reset link is on its way.",
    accessRestricted: "Access is restricted. If you don't have an invite code, contact the administrator.",
    noAccountYet: "No account yet? Create one",
    captchaRequired: "Complete the security check before continuing.",
    captchaFailed: "Security check failed. Please try again.",
    invalidCredentials: "Invalid email or password.",
    checkEmailTitle: "Check your email",
    checkEmailSubtitle: "We sent a confirmation link. One click and you're in the dashboard.",
    checkEmailSent: "Confirmation link sent.",
    checkEmailBody: "We sent an email to {{email}}. Click the link to activate your account — you'll be signed in automatically.",
    backToSignIn: "Back to sign in",
    emailNotConfirmed: "Confirm your email using the link we sent before signing in.",
    accessOpenNote: "Create a free account. Campaigns and domains unlock after you choose a plan.",
    invisibleProtection: "Invisible protection.",
    realResults: "Real results.",
    authDescription: "Filter bots and moderators in real time. Your visitors see the offer, everyone else sees the safe page.",
    detection: "Detection",
    detectionFull: "Bot Detection",
    latency: "Latency",
  },
  gate: {
    profileMissing: "Your account is being prepared. If this persists, contact support.",
    fetchError: "We couldn't verify your account right now. Some actions stay locked.",
    retry: "Try again",
    contactSupport: "Support",
  },

  // Update password (recovery flow)
  updatePassword: {
    title: "Set a new password",
    subtitle: "Choose a strong password to regain access to your account.",
    sidebarTitle: "Secure your account",
    sidebarDescription: "One last step. Set a new password and jump back into protecting your campaigns.",
    newPasswordLabel: "New Password",
    confirmPasswordLabel: "Confirm New Password",
    submitButton: "Update Password",
    passwordsMatch: "Passwords match",
    passwordsDoNotMatch: "Passwords do not match",
    updateSuccess: "Password updated. Redirecting you to your dashboard…",
    updateFailed: "Unable to update password. The reset link may have expired.",
    validatingSession: "Validating reset link…",
  },

  // Landing
  landing: {
    smartProtection: "Smart protection for your links",
    heroTitle: "Protect your campaigns.",
    heroTitleHighlight: "Maximize your results.",
    heroDesc: "CloakerX filters bots, moderators, and unwanted traffic in real time, ensuring only real visitors reach your offer.",
    createFreeAccount: "Create Free Account",
    seeFeatures: "See Features",
    login: "Login",
    getStarted: "Get Started",
    goToDashboard: "Go to Dashboard",
    everythingYouNeed: "Everything you need",
    everythingDesc: "Powerful tools to protect and optimize your campaigns.",
    advancedCloaking: "Advanced Cloaking",
    advancedCloakingDesc: "Automatically redirect bots and moderators to safe pages.",
    proxyDetection: "Proxy/VPN Detection",
    proxyDetectionDesc: "Identify and block suspicious traffic with IP intelligence APIs.",
    ultraLowLatency: "Ultra-Low Latency",
    ultraLowLatencyDesc: "Edge Functions process each request in milliseconds without slowing your funnel.",
    multiPlatform: "Multi-Platform",
    multiPlatformDesc: "Native support for TikTok, Facebook, and Google Ads.",
    whyChoose: "Why choose CloakerX?",
    whyChooseDesc: "Built for affiliates and media buyers who need security and performance without complexity.",
    benefit1: "Protect your campaigns against manual reviews",
    benefit2: "Complete dashboard with real-time metrics",
    benefit3: "Setup in less than 5 minutes",
    benefit4: "Support for multiple domains and campaigns",
    botDetectionRate: "Bot detection rate",
    avgLatency: "Average latency per request",
    readyToProtect: "Ready to protect your campaigns?",
    readyDesc: "Start for free and see the difference in minutes.",
    footer: "Smart protection for media buyers.",
  },

  // 404
  notFound: {
    title: "404",
    message: "Oops! Page not found",
  },

  // Admin
  admin: {
    title: "Admin & Users",
    subtitle: "Manage invite codes, promo codes, and registered users.",
    inviteCodes: "Invite Codes",
    registeredUsers: "Registered Users",
    promoCodes: "Promo Codes",
    total: "Total",
    available: "Available",
    used: "Used",
    createNewCode: "Create new code",
    generateRandom: "Generate Random",
    customCodePlaceholder: "Custom code (e.g. VIP-2024)",
    code: "Code",
    usedBy: "Used By",
    usedAt: "Used At",
    noCodes: "No codes created yet.",
    codeCreated: "Code created successfully!",
    codeDeleted: "Code deleted.",
    codeDuplicate: "This code already exists.",
    codeMinLength: "Code must be at least 4 characters.",
    userEmail: "User Email",
    registrationDate: "Registration Date",
    totalClicks: "Total Clicks",
    manage: "Manage",
    managePlan: "Manage Plan",
    changePlanFor: "Change plan for",
    selectPlan: "Select Plan",
    savePlan: "Save Plan",
    planUpdated: "Plan updated to {{plan}} for {{email}}",
    noUsers: "No registered users found.",
    createPromo: "Create Promo Code",
    promoCode: "Promo Code",
    targetPlan: "Target Plan",
    durationDays: "Duration (days)",
    maxUses: "Max Uses",
    uses: "Uses",
    promoCreated: "Promo code created!",
    promoDuplicate: "This promo code already exists.",
    promoMinLength: "Code must be at least 3 characters.",
    promoUpdated: "Promo code updated.",
    promoDeleted: "Promo code deleted.",
    noPromos: "No promo codes created yet.",
    domains: "Domains",
    campaigns: "Campaigns",
    expiresAt: "Expires",
    expired: "Expired",
    filterByPlan: "Filter by plan",
    allPlans: "All Plans",
    resetBilling: "Reset Billing",
    billingReset: "Billing reset for this user.",
    commandCenter: "Admin Command Center",
    totalUsers: "Total Users",
    activeCampaigns: "Active Campaigns",
    clicksThisCycle: "Clicks This Cycle",
    allUsers: "All Users",
    email: "Email",
    plan: "Plan",
    clickUsage: "Click Usage",
    registered: "Registered",
    statusLabel: "Status",
    suspended: "Suspended",
    activeStatus: "Active",
    reactivate: "Reactivate",
    suspendUser: "Suspend User",
    changePlan: "Change Plan",
    changePlanTitle: "Change Plan — {{email}}",
    userSuspended: "User suspended",
    userReactivated: "User reactivated",
    planUpdatedSuccess: "Plan updated",
    // === Turn A.2 — Commercial Visibility v2 ===
    lastLogin: "Last Login",
    lastActivity: "Last Activity",
    daysAgo: "{{count}}d ago",
    today: "Today",
    yesterday: "Yesterday",
    neverLoggedIn: "Never",
    noActivity: "No activity",
    usageStatus: "Usage Status",
    churnRisk: "Churn Risk",
    // Usage status — 5 buckets
    statusSafe: "Healthy",
    statusWarning: "Warning",
    statusCritical: "Critical",
    statusOverage: "Overage",
    statusInactive: "Inactive",
    // Churn risk — 4 levels
    churnLow: "Low",
    churnMedium: "Medium",
    churnHigh: "High",
    churnCritical: "Critical",
    churnNone: "—",
    // Filters and sorting
    filterByStatus: "Filter by status",
    filterByChurn: "Filter by risk",
    allStatuses: "All statuses",
    allChurnLevels: "All risk levels",
    sortBy: "Sort",
    activeFilters: "{{count}} active filters",
    // Empty states
    noUsersFound: "No users match the applied filters.",
    noUsersFoundHint: "Try adjusting or clearing the filters to see all users.",
    noUsersAtAll: "No users registered yet.",
    noUsersAtAllHint: "When someone creates an account, they will appear here.",
    // Tooltip / metadata
    signupDays: "{{count}} days on platform",
    clicksOf: "{{current}} of {{max}} clicks",
    domainsCount: "{{count}} domains",
    campaignsCount: "{{count}} campaigns",
    // === Turn B.1 — Command Center ===
    commandCenterSubtitle: "Commercial visibility, invites and promo codes in one place",
    tabVisibility: "Visibility",
    tabInvites: "Invites",
    tabPromos: "Promos",
    placeholderLabel: "Tab under construction",
    placeholderHint: "This tab's content will be migrated in the next turn (B.2). The orchestrator is already functional.",
    // === Turno B.2.5.3 — Invite Codes B2B Hardened ===
    totalCodes: "Total Codes",
    availableCapacity: "Available Capacity",
    usedCapacity: "Used Capacity",
    maxUsesLabel: "Limit",
    maxUsesHint: "Set how many people can use this code (1-1000). Default: 1.",
    maxUsesRange: "Usage limit must be between 1 and 1000.",
    codeInvalidFormat: "Invalid code. Use 4-50 characters: A-Z, 0-9, hyphen.",
    codeUpdated: "Code updated.",
    unauthorized: "Access denied.",
    usesColumn: "Uses",
    statusActive: "Active",
    statusExhausted: "Exhausted",
    redemptionsTitle: "Redemptions of {{code}}",
    redemptionsSubtitle: "List of people who used this invite code.",
    noRedemptions: "No redemptions yet for this code.",
    unknownEmail: "Unknown email",
    userDeleted: "User deleted",
    lastSignIn: "Last sign-in",
    deleteCodeTitle: "Delete invite code?",
    deleteCodeDescription: "Are you sure you want to delete code \"{{code}}\"? This action is irreversible and removes all associated redemption records.",
    auditLogTitle: "Audit Log",
  auditLogSubtitle: "History of administrative and security actions",
  filters: "Filters",
  clearFilters: "Clear filters",
  filterByAction: "Filter by action",
  allActions: "All actions",
  filterByAdmin: "Filter by admin",
  allAdmins: "All admins",
  dateRange: "Date range",
  allTime: "All time",
  last24h: "Last 24 hours",
  last7d: "Last 7 days",
  last30d: "Last 30 days",
  searchAuditPlaceholder: "Search by email, action or target...",
  noAuditLogs: "No audit logs found",
  noAuditLogsFiltered: "No results for the applied filters",
  when: "When",
  adminCol: "Admin",
  actionCol: "Action",
  targetCol: "Target",
  ipCol: "IP",
  detailsCol: "Details",
  viewDetails: "View details",
  loadMore: "Load more",
  auditDetailsTitle: "Log details",
  userAgent: "User Agent",
  payload: "Payload",
},

  // Analytics
  analytics: {
    title: "Analytics",
    subtitle: "Campaign performance tracking & insights",
    selectCampaign: "Select a campaign",
    today: "Today",
    last7days: "Last 7 days",
    last30days: "Last 30 days",
    allTime: "All time",
    emptySelect: "Select a campaign to view tracking data",
    emptyData: "No clicks recorded yet.",
    emptyDataHint: "Set up the URL with tracking macros in your ad manager.",
    totalClicks: "Total Clicks",
    uniqueClicks: "Unique Clicks",
    approvedLeads: "Approved Leads",
    realTraffic: "Real Traffic (Approved)",
    approvalRate: "Approval Rate",
    totalCost: "Total Cost",
    cpl: "CPL",
    dailyChart: "Daily approved vs blocked clicks",
    insufficientChartData: "Insufficient data for the chart. Keep running traffic to see the daily evolution here.",
    platformBreakdown: "Breakdown by Platform",
    countryBreakdown: "Breakdown by Country",
    deviceBreakdown: "Breakdown by Device",
    platform: "Platform",
    clicks: "Clicks",
    approved: "Approved",
    blocked: "Blocked",
    cost: "Cost",
    avgCpc: "Avg CPC",
    country: "Country",
    device: "Device",
    avgScore: "Avg Score",
    threatAnatomy: "Threat Anatomy",
    noBlockData: "No blocked traffic to analyze.",
    roiSaved: "Protected ROI",
    roiSavedTooltip: "Estimated savings based on blocked clicks × average cost per click.",
    totalRevenue: "Total Revenue",
    realRoi: "Real ROI",
    conversions: "Conversions",
  },

  dashboard2: {
    healthScore: "Campaign Health",
    healthGood: "Excellent",
    healthWarning: "Warning",
    healthDanger: "Critical",
    healthTooltip: "Average risk score of approved clicks. Lower is cleaner traffic.",
  },

  // Languages
  languages: {
    en: "English",
    pt: "Português",
    es: "Español",
  },
  blockReason: {
    suspicious_behavior: "Suspicious behavior",
    device_not_allowed: "Device not allowed",
    geo_blocked: "Geographic origin blocked",
    invalid_click: "Invalid click identifier",
    campaign_inactive: "Campaign paused",
    suspicious_connection: "Suspicious connection",
    blocked_source: "Blocked source",
    blocked_generic: "Blocked",
  },
  overage: {
    modalTitle: "🚀 Your campaign is taking off!",
    modalDesc: "You've reached the click limit on your current plan. But don't worry — we haven't paused your traffic. You're still receiving leads and making sales! We're only charging a small overage fee to keep your scale running.",
    modalBtn: "Settle Invoice / Upgrade Plan",
    bannerText: "⚠️ You've reached your plan limit. Your traffic is still active and being billed as overage.",
    bannerBtn: "View Plans",
  },
};

export default en;
