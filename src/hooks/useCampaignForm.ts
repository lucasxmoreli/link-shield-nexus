...
export function useCampaignForm() {
  ...
  const [prelanderEnabled, setPrelanderEnabled] = useState(false);
  const [prelanderHeadline, setPrelanderHeadline] = useState("");
  const [prelanderBody, setPrelanderBody] = useState("");
  const [prelanderCta, setPrelanderCta] = useState("");
  ...
  useEffect(() => {
    if (campaign) {
      ...
      setPrelanderEnabled(campaign.prelander_enabled ?? false);
      setPrelanderHeadline(campaign.prelander_headline ?? "");
  ...
  const prelanderValid =
    !prelanderEnabled || (!!prelanderHeadline.trim() && !!prelanderBody.trim());
  const prelanderHeadlineError = attemptedSave && prelanderEnabled && !prelanderHeadline.trim();
  const prelanderBodyError = attemptedSave && prelanderEnabled && !prelanderBody.trim();
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload: Record<string, unknown> = {
        ...
        prelander_enabled: prelanderEnabled,
        prelander_headline: prelanderEnabled ? prelanderHeadline.trim() || null : null,
        prelander_body: prelanderEnabled ? prelanderBody.trim() || null : null,
        prelander_cta: prelanderEnabled ? prelanderCta.trim() || null : null,
